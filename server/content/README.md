...
| File           | What it holds                                              |
| -------------- | ---------------------------------------------------------- |
| `units.json`   | Unit definitions: identity, kingdom, fixed tier, maximum armour, Mounted |
| `weapons.json` | Structural weapon metadata: canonical type and wield slot cost |
...
- **Combat stats.** Power, Critical Chance, Weight and Range are the weapon registry's, and
  Task 6 owns reading them. `weapons.json` carries only what equipping needs.
- **Armour items.** `MaxArmor` is a Unit's structural limit and is published by the API, but no
  armour item content exists yet and none is invented here.
...
a slot cost outside 1–2 or a missing display name all fail validation with the offending entry
in the message.
