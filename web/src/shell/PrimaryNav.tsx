...
function NavItem({ destination, pinned }: { destination: ShellDestination; pinned?: boolean }) {
  return (
    <li className={`flex-1 lg:flex-none ${pinned ? "lg:mt-auto lg:border-t-2 lg:border-slate/70 lg:pt-4" : ""}`}>
      <NavLink
        to={destination.path}
        className={({ isActive }) =>
          [
            "group relative flex h-full min-h-14 items-center justify-center gap-3 px-3 font-hud text-[0.875rem]",
            "font-semibold uppercase tracking-[0.16em] transition-colors motion-reduce:transition-none",
            "focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-selected",
            "lg:min-h-12 lg:justify-start lg:px-6",
            isActive
